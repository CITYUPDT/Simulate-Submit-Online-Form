...
def fetch_basic_info():
    ...
    data.test_start_datetime = datetime.strptime(basic_info["测试开始日期"][0], '%Y-%m-%d %H:%M')
    data.test_start_timestamp = int(time.mktime(data.test_start_datetime.timetuple()))
    data.test_end_datetime = datetime.strptime(basic_info["测试结束日期"][0], '%Y-%m-%d %H:%M')
    data.test_end_timestamp = int(time.mktime(data.test_end_datetime.timetuple()))

    data.real_url = basic_info["正式链接"][0]
    data.real_start_datetime = datetime.strptime(basic_info["报名开始日期"][0], '%Y-%m-%d %H:%M')
